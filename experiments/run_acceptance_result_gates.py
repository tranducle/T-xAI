...
def main()->int:
    ...
    g6={
      ...
      'metrics':{'E10_rows':len(rows10),'E10_max_ratio_L1':max_ratio,'E10_all_L1_hold':all(r['holds_L1'] for r in rows10),'E10_all_dobrushin_hold':all(r['holds_dobrushin'] for r in rows10),'E11_attack_types':len(rows11),'E11_min_authenticated_detection_rate':min_auth,'E11_clean_false_reject_rate':float(e11['gate']['metrics']['clean_false_reject_rate']),'E11_compromised_key_bypass_rate':float(e11['boundary_control']['compromised_key_bypass_rate']),'all_selected_metrics_finite':all(math.isfinite(v) for v in vals)},
      'pass_criteria':{'E10_official_sample':500,'E10_bound_rows':'all 12 rows satisfy generic and Dobrushin inequalities','E10_nontrivial_tightness':'0.1 < max LHS/RHS < 1','E11_official_sample':500,'E11_detection':'>=0.99 for all tested attacks','E11_clean_acceptance':'false reject <=0.01','E11_boundary_control':'>=0.99 bypass under authenticator compromise'},
